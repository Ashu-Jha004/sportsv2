...
import { Search, X, Shield, AlertCircle, Filter } from "lucide-react";
...
export function ReceivedChallengeFilterHeader() {
  ...
  useEffect(() => {
    const checkPerms = async () => {
      try {
        ...
        console.error(
          "❌ [ReceivedChallengeFilterHeader] Permission check failed:",
          error
        );
        setPermissions({
          canChallenge: false,
  ...
  const updateURLParams = useCallback(
    (filters: Record<string, string | undefined>) => {
      ...
      router.push(`/challenges/received?${params.toString()}`, {
        scroll: false,
      });
    },
    [router, searchParams]
  ...
  const handleSearch = useCallback(() => {
    try {
      ...
      console.error("❌ [ReceivedChallengeFilterHeader] Search error:", error);
    }
  ...
  const handleClearFilters = useCallback(() => {
    try {
      ...
      console.error(
        "❌ [ReceivedChallengeFilterHeader] Clear filters error:",
        error
      );
    }
  }, [clearFilters, router]);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Review and respond to match challenges from other teams
              </p>
            </div>
          </div>
