...
export function CounterProposeDialog() {
  ...
  // Get challenge directly from store
  ...
  React.useEffect(() => {
    if (challenge) {
      console.log(
        "📝 [CounterProposeDialog] Initializing form with challenge:",
        challenge
      );
      setDate(
  ...
  const handleCounterPropose = useCallback(async () => {
    if (!selectedMatchId) return;

    if (!validateForm()) {
      console.warn("⚠️ [CounterProposeDialog] Form validation failed");
      return;
    }
    ...
    try {
      ...
      console.log(
        "✏️ [CounterProposeDialog] Sending counter-proposal:",
        selectedMatchId
      );
      ...
      console.error(
        "❌ [CounterProposeDialog] Error sending counter-proposal:",
        error
      );
      toast("Error");
    } finally {
  ...
  // Show error if no challenge data
  if (!challenge) {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          Propose different match details. The challenger team will be able to
          accept or suggest further changes until you both agree.
        </AlertDescription>
      </Alert>
      ...
      <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg border">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-bold truncate">{challenge.challengerTeamName}</h3>
      ...
      {/* Message */}
      <div className="space-y-2">
        <Label htmlFor="message">Message to Challenger Team (Optional)</Label>
        <Textarea
          id="message"
          value={message}
      ...
      <Alert>
        ...
        <AlertDescription className="text-xs">
          The challenger team will receive your counter-proposal and can accept
          it or suggest further changes. You can continue negotiating until both
          teams agree.
        </AlertDescription>
      </Alert>
